...
function visualizeCirculantGraph(nVertex, param) {
  const [adjacencyMatrix, lmd] = findLmd(parseInt(nVertex), param);
  ...
  for (let i = 0; i < nVertex; i++) {
    ...
    graph.addNode(`v${i}`, { x: 1*absis, y: 1*ordinat, label: `v${i}`, color: lmd.includes(i) ? "red": "blue", size: (130-10 * Math.floor(nVertex/10))/100 * 10 });
  }
  ...
  for (let j = 0; j < nVertex; j++) {
    for (let k = 0; k < nVertex; k++) {
      if (adjacencyMatrix[j][k] == 1) {
        ...
        graph.addEdge(`v${j}`, `v${(k)%nVertex}`, { color: "#000", size: (120-10 * Math.floor(nVertex/10))/100 * 5 });
      }
    }
  }
